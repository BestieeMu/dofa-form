...
export default function StepForm2() {
  ...
  return (<section className="card">
    <div className="step-header"><div className="step-icon">🆘</div><div><h2>Form 2 — Emergency Contact & Guardian Information</h2><p className="step-desc">Who should we contact in an emergency?</p></div></div>
    <div className="grid">
      <div><label className="required">Emergency Contact Full Name</label><input className={ec('emergency.contactFullName')} name="emergency.contactFullName" value={getDeep('emergency.contactFullName') || ''} onChange={handleChange} required /><E name="emergency.contactFullName" /></div>
      <div><label className="required">Relationship to Participant</label><input className={ec('emergency.relationship')} name="emergency.relationship" value={getDeep('emergency.relationship') || ''} onChange={handleChange} required /><E name="emergency.relationship" /></div>
      <div><label className="required">Primary Phone</label><input className={ec('emergency.primaryPhone')} name="emergency.primaryPhone" value={getDeep('emergency.primaryPhone') || ''} onChange={handleChange} required /><E name="emergency.primaryPhone" /></div>
    ...
    <div className="grid">
      <div><label>Has Legal Guardian?</label><select name="emergency.hasGuardian" value={getDeep('emergency.hasGuardian') || ''} onChange={handleChange}><option value=""> </option><option>Yes</option><option>No</option></select></div>
      ...
      <div><label>Mailing Address</label><input name="emergency.guardianMailingAddress" value={getDeep('emergency.guardianMailingAddress') || ''} onChange={handleChange} /></div>
