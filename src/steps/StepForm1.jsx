...
export default function StepForm1() {
  ...
  return (<section className="card">
    <div className="step-header"><div className="step-icon">4</div><div><h2>Participant Information & Demographics</h2><p className="step-desc">Complete the participant details shown on the revised template</p></div></div>
    <h3>Personal Information</h3>
    <div className="grid">
      <div><label className="required">Full Legal Name</label><input className={ec('participant.fullLegalName')} name="participant.fullLegalName" value={getDeep('participant.fullLegalName') || ''} onChange={handleChange} /><E name="participant.fullLegalName" /></div>
      ...
      <div><label className="required">Date of Birth</label><input className={ec('participant.dateOfBirth')} type="date" name="participant.dateOfBirth" value={getDeep('participant.dateOfBirth') || ''} onChange={handleChange} /><E name="participant.dateOfBirth" /></div>
      <div><label>Full Social Security Number</label><input name="participant.ssnFull" value={getDeep('participant.ssnFull') || ''} onChange={handleChange} /></div>
    ...
    <div className="grid">
      <div><label className="required">Current Address</label><input className={ec('participant.currentAddress')} name="participant.currentAddress" value={getDeep('participant.currentAddress') || ''} onChange={handleChange} /><E name="participant.currentAddress" /></div>
      <div><label className="required">Primary Phone</label><input className={ec('participant.primaryPhone')} name="participant.primaryPhone" value={getDeep('participant.primaryPhone') || ''} onChange={handleChange} /><E name="participant.primaryPhone" /></div>
